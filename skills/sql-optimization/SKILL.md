...
This app uses **Knex.js** as its query builder. When suggesting optimized queries, always prefer Knex syntax over raw SQL strings unless the operation is not expressible with Knex's fluent API.

### Syntax preference rules
- ✅ **Use Knex fluent API** for all standard queries: selects, joins, wheres, aggregates, inserts, updates, deletes.
- ✅ **Use `knex.raw()`** only for database-specific expressions that Knex cannot express natively (e.g. window functions, complex `CASE` expressions, vendor-specific index hints).
- ❌ **Never** write a full raw SQL string when the equivalent Knex chain is available.

### Examples
```js
// ❌ BAD: raw SQL when Knex can handle it
knex.raw('SELECT id, name FROM users WHERE status = ? AND created_at > ?', ['active', date]);

// ✅ GOOD: Knex fluent API
knex('users')
    .select('id', 'name')
    .where('status', 'active')
    .where('created_at', '>', date);

// ✅ GOOD: knex.raw() only for what Knex can't express natively
knex('products')
    .select('*')
    .select(knex.raw('AVG(price) OVER (PARTITION BY category_id) as avg_category_price'));
```
...
3. **Optimize**: Apply appropriate optimization techniques — write solutions in **Knex fluent API** syntax; fall back to `knex.raw()` only when necessary
